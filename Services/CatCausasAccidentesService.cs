    ...
    public class CatCausasAccidentesService : ICatCausasAccidentesService
    {
        ...
        public List<CatCausasAccidentesModel> ObtenerCausasActivas()
        {
            ...
            using (SqlConnection connection = new SqlConnection(_sqlClientConnectionBD.GetConnection()))
                ...
                {
                    connection.Open();
                    SqlCommand command = new SqlCommand("SELECT c.*, e.estatus FROM catCausasAccidentes AS c INNER JOIN estatus AS e ON c.estatus = e.estatus" +
                                                        " WHERE c.estatus = 1 ORDER BY CausaAccidente ASC;", connection);
